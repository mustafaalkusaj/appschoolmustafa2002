...
<script>window.addEventListener("load",()=>{window.print();})</script>
  ...
  const handleExcelExport = () => {
    ...
    const data = teachers.map((t) => ({
      [isEnLocal ? "Employee ID" : "رقم الموظف"]: t.employee_id ?? "",
      [isEnLocal ? "Full Name" : "الاسم الكامل"]: t.full_name,
      [isEnLocal ? "Subject" : "المادة"]: t.subject ?? "",
      [isEnLocal ? "Job Title" : "المسمى الوظيفي"]: t.job_title ?? "",
      [isEnLocal ? "Contract Type" : "نوع العقد"]: contractLabel(t.contract_type),
      [isEnLocal ? "Phone" : "الهاتف"]: t.phone ?? "",
      [isEnLocal ? "Email" : "البريد"]: t.email ?? "",
      [isEnLocal ? "Gender" : "الجنس"]: genderLabel(t.gender),
      [isEnLocal ? "Hire Date" : "تاريخ التعيين"]: t.hire_date ?? "",
      [isEnLocal ? "Years of Experience" : "سنوات الخبرة"]: t.years_experience ?? "",
      [isEnLocal ? "Status" : "الحالة"]: statusLabel(t.status),
      [isEnLocal ? "App Account" : "حساب التطبيق"]: t.app_username ?? "",
    }));

    const ws = XLSX.utils.json_to_sheet(data);
    const wb = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(wb, ws, isEnLocal ? "Teachers" : "الأساتذة");
    XLSX.writeFile(wb, isEnLocal ? "teachers.xlsx" : "الأساتذة.xlsx");
  };
