...
export function RolesTab({
  ...
  return (
    <div className="space-y-6">
      {message ? (
        <MigrationNotice title="تنبيه" description={message} />
      ) : null}
      ...
      {activeTab === "matrix" ? (
        <SectionCard
          ...
          description="نظرة شاملة على الصلاحيات الممنوحة لكل دور افتراضي في النظام."
        >
        ...
        <SectionCard
          title={`الأدوار المخصصة (${roles.length})`}
          description="أنشئ قوالب صلاحيات ومسميات وظيفية إضافية مثل محاسب أو مسؤول حضور، مبنية على دور أساسي مع حزمة صلاحيات مستقلة لكل مدرسة أو على مستوى المنصة."
          actions={
            <button
              type="button"
        ...
        >
          {loading ? (
            <div className="space-y-3 py-10">
              {[...Array(3)].map((_, index) => (
                <div key={index} className="sk h-28 w-full rounded-[24px]" />
              ))}
            </div>
          ) : roles.length === 0 ? (
            <EmptyState
              ...
              description="يمكنك إنشاء أدوار إضافية فوق الأدوار الافتراضية وتخصيص صلاحياتها لكل مدرسة."
            ...
            <div className="grid gap-4 lg:grid-cols-2">
              {roles.map((role) => {
                ...
                return (
                  <article key={role.id} className="ui-surface rounded-[28px] p-5">
                    <div className="mb-4 flex items-start justify-between gap-3">
                      <div className="space-y-1">
                        <h3 className="text-lg font-black text-[var(--text-primary)]">{role.name}</h3>
                        <p className="text-sm leading-7 text-[var(--text-secondary)]">
                          {role.description || "بدون وصف إضافي"}
                        </p>
                      </div>
                      <span className="ui-pill">{ROLE_LABELS[role.base_role]}</span>
                    </div>

                    <div className="mb-4 flex flex-wrap gap-2">
                      <span className="ui-pill">
                        {role.school_id ? relationName(role.schools) || "مدرسة محددة" : "على مستوى المنصة"}
                      </span>
                      <span className="ui-pill ui-pill--success">{permissions.length} صلاحيات</span>
                    </div>

                    <div className="rounded-[22px] border border-[var(--border)] bg-[var(--surface-muted)] p-4">
                      <div className="mb-3 text-xs font-black text-[var(--text-tertiary)]">ملخص الصلاحيات</div>
                      <div className="flex flex-wrap gap-2">
                        {permissions.slice(0, 6).map((permission) => (
                          <span key={permission} className="ui-pill text-[10px]">
                            {permission}
                          </span>
                        ))}
                        {permissions.length > 6 ? (
                          <span className="ui-pill text-[10px]">+{permissions.length - 6}</span>
                        ) : null}
                      </div>
                    </div>
      ...
      {showForm ? (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4 backdrop-blur-sm">
          <div className="ui-surface w-full max-w-3xl rounded-[32px] p-6 shadow-2xl">
            ...
            <form onSubmit={handleSubmit} className="space-y-5">
              <div className="grid gap-4 md:grid-cols-2">
                <div>
                  <label className="mb-2 block text-xs font-black">اسم الدور</label>
                  <input
                    className="ui-input"
                    required
                    value={formData.name}
                ...
                <div>
                  <label className="mb-2 block text-xs font-black">الدور الأساسي</label>
                  <select
                    className="ui-input"
                ...
                <div className="md:col-span-2">
                  <label className="mb-2 block text-xs font-black">الوصف</label>
                  <textarea
                ...
                <div className="md:col-span-2">
                  <label className="mb-2 block text-xs font-black">النطاق</label>
                  <select
                    className="ui-input"
              ...
              <div className="rounded-[28px] border border-[var(--border)] bg-[var(--surface-muted)] p-4">
                <div className="mb-4 flex items-center justify-between gap-3">
                  <div>
                    <h4 className="text-base font-black text-[var(--text-primary)]">الصلاحيات</h4>
                    <p className="text-sm leading-7 text-[var(--text-secondary)]">
                      تم ملء الصلاحيات الافتراضية من الدور الأساسي ويمكنك تعديلها قبل الحفظ.
                    </p>
                  </div>
                  <button
                    type="button"
