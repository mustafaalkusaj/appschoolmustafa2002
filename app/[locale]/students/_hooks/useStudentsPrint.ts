...
import { buildPrintableCardHtml, buildSingleStudentPrintHtml, buildStudentPrintCardHtml, generateLoginQrDataUrl } from "../_utils";
...
export function useStudentsPrint(options: UseStudentsPrintOptions) {
  ...
  const openAccountCardWindow = useCallback(async (card: ManagedUserAccountCard, autoPrint = true, revealedPassword?: string | null) => {
    ...
    const qrDataUrl = await generateLoginQrDataUrl(card.login_identifier, passwordToShow);
