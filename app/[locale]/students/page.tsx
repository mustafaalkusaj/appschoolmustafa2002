      ...
      <head>
        ...
        <style>
          ...
          .header { 
            ...
            border-bottom: 3px solid #0F5D91; 
          }
          .header h1 { 
            font-size: 28px; 
            font-weight: 900; 
          ...
          .stat-box { 
            background: linear-gradient(135deg, #E6F6FF, #D4F1FF); 
            border-radius: 16px; 
            padding: 1rem; 
          }
          .stat-label { 
...
function handlePrint(s: StudentWithFees){
  ...
  function handleFileChange(e:React.ChangeEvent<HTMLInputElement>){
    ...
    reader.onload=async(ev)=>{
      try{
        const XLSX = await loadXLSX();
        const wb=await XLSX.read(ev.target?.result,{type:"binary"});
  ...
  <>
    <style>{`
      *,*::before,*::after{box-sizing:border-box;margin:0;padding:0}
      ...
      .sc{background:white;border-radius:12px;padding:.8rem 1rem;border:1px solid rgba(22,137,201,0.06);box-shadow:0 2px 8px rgba(22,137,201,0.06)}
      ...
      .filter-sel{padding:.55rem .8rem;background:white;border:1px solid rgba(22,137,201,0.12);border-radius:9px;font-family:var(--font-manrope),Segoe UI,sans-serif;font-size:.8rem;outline:none;color:var(--dark)}
      ...
      /* TABLE */
      .tbl-wrap{background:white;border-radius:13px;border:1px solid rgba(22,137,201,0.06);overflow:hidden;box-shadow:0 2px 8px rgba(22,137,201,0.06)}
      ...
      .btn-action:hover{background:#D7F1FF}
      ...
      .d-item:hover{background:#F6FBFF}
      ...
      .fi,.fs{padding:.65rem .85rem;background:#F6FBFF;border:1.5px solid rgba(22,137,201,0.12);border-radius:9px;font-family:var(--font-manrope),Segoe UI,sans-serif;font-size:.82rem;direction:rtl;outline:none}
      ...
      .upload-area{border:2px dashed rgba(22,137,201,0.3);border-radius:12px;padding:1.5rem;text-align:center;cursor:pointer;background:#FAFAFE;margin-bottom:1rem}
      .upload-area:hover{border-color:var(--p3);background:#EEF8FF}
      .upload-area svg{width:36px;height:36px;color:var(--p4);margin-bottom:.5rem}
      .template-btn{display:inline-flex;align-items:center;gap:.4rem;padding:.5rem 1rem;background:#E6F6FF;color:var(--p3);border:none;border-radius:8px;font-family:var(--font-manrope),Segoe UI,sans-serif;font-size:.78rem;font-weight:700;cursor:pointer;margin-bottom:1rem}
      .preview-table{width:100%;border-collapse:collapse;font-size:.72rem;margin-bottom:.5rem}
      .preview-table th{background:#F6FBFF;padding:.4rem .6rem;text-align:left;font-weight:700;color:var(--p2);border-bottom:1px solid rgba(22,137,201,0.1)}
      .preview-table td{padding:.4rem .6rem;border-bottom:1px solid rgba(22,137,201,0.05)}
      .cols-info{background:#EEF8FF;border-radius:10px;padding:.8rem 1rem;margin-bottom:1rem}
                  ...
                  <>
                    ...
                    {totalPages > 1 && (
                      <div className="pagination flex items-center justify-between p-4 border-t border-slate-200 bg-slate-50 dark:border-slate-700 dark:bg-slate-800/50">
                        <button className="btn-nav disabled:opacity-50 px-4 py-2 bg-gradient-to-l from-cyan-400 to-blue-700 text-white rounded-lg cursor-pointer hover:shadow-md transition-all disabled:cursor-not-allowed" disabled={page === 1} onClick={()=>setPage(page - 1)}>
                          السابق
                        </button>
                        <span className="font-bold text-slate-800 dark:text-slate-200">
                          صفحة {page} من {totalPages} | {totalCount} طالب
                        </span>
                        <button className="btn-nav disabled:opacity-50 px-4 py-2 bg-gradient-to-l from-cyan-400 to-blue-700 text-white rounded-lg cursor-pointer hover:shadow-md transition-all disabled:cursor-not-allowed" disabled={page === totalPages} onClick={()=>setPage(page + 1)}>
                          التالي
                        </button>
                      </div>
                    )}
            ...
            <div className="fg">
              ...
              <div className="ff">
                ...
                {form.class_name&&form.class_name!=="__manual__"&&(()=>{
                  ...
                  if(!cf)return null;
                  return(
                    <div style={{background:"linear-gradient(135deg,#E6F6FF,#D4F1FF)",borderRadius:8,padding:".45rem .7rem",marginTop:".4rem",fontSize:".72rem",color:"var(--p2)",fontWeight:700}}>
