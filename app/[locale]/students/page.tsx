                    ...
                    <table>
                      ...
                      <tbody>
                        {pagedStudents.map((s,i)=>{
                          ...
                          return <tr key={s.id}>
                            <td style={{color:"var(--text-tertiary)",fontSize:".7rem"}}>{(page - 1) * pageSize + i + 1}</td>
                            <td>
                              <span
                                className="student-name"
                                style={{fontWeight:700,color:"var(--primary)",cursor:"pointer",textDecoration:"underline",textUnderlineOffset:"3px"}}
                                onClick={(e)=>openMenu(e,s)}
                              >{s.full_name}</span>
                            </td>
                            <td>{s.class_name}</td>
                            <td>{s.section||"—"}</td>
                            <td style={{color:"var(--text-tertiary)"}}>{s.address||"—"}</td>
                            <td style={{color:"var(--text-tertiary)"}}>{s.phone||"—"}</td>
            ...
            <div className="fg">
              ...
              <div className="ff">
                ...
                {form.class_name&&form.class_name!=="__manual__"&&(()=>{
                  ...
                  if(!cf)return null;
                  return(
                    <div style={{background:"linear-gradient(135deg,var(--primary-subtle),#E0D8F8)",borderRadius:8,padding:".45rem .7rem",marginTop:".4rem",fontSize:".72rem",color:"var(--primary)",fontWeight:700}}>
      ...
      <div className="overlay">
        <div className="modal modal-sm">
          ...
          <div className="del-msg">
            ...
            <span style={{color:"var(--text-tertiary)",fontSize:".8rem"}}>سيتم نقله لقائمة المحذوفين ويمكن استعادته لاحقاً</span>
          </div>
          <div className="fa">
        ...
        <div className="modal modal-lg">
          ...
          <div className="cols-info">
            ...
            <div className="cols-grid">
              ...
              <div className="col-item"><span style={{color:"var(--text-tertiary)"}}>○</span> العنوان</div>
              <div className="col-item"><span style={{color:"var(--text-tertiary)"}}>○</span> الهاتف</div>
              <div className="col-item"><span style={{color:"var(--text-tertiary)"}}>○</span> هاتف ولي الأمر</div>
              <div className="col-item"><span style={{color:"var(--text-tertiary)"}}>○</span> إجمالي الرسوم</div>
              <div className="col-item"><span style={{color:"var(--text-tertiary)"}}>○</span> المدفوع</div>
            </div>
          </div>
          ...
          <div className="upload-area" onClick={()=>fileRef.current?.click()}>
            ...
            <div style={{fontSize:".75rem",color:"var(--text-tertiary)"}}>xlsx أو xls</div>
        ...
        <div className="modal modal-lg">
          ...
          <div style={{display:"grid",gap:"1rem",gridTemplateColumns:"repeat(auto-fit,minmax(220px,1fr))",marginBottom:"1rem"}}>
            <div style={{background:"#F8FBFF",border:"1px solid rgba(15,91,141,0.12)",borderRadius:18,padding:"1rem"}}>
              <div style={{fontSize:".75rem",fontWeight:800,color:"var(--text-tertiary)"}}>الاسم الكامل</div>
              <div style={{marginTop:".4rem",fontSize:"1.05rem",fontWeight:900,color:"var(--primary)"}}>{accountCard.full_name}</div>
            </div>
            <div style={{background:"#F8FBFF",border:"1px solid rgba(15,91,141,0.12)",borderRadius:18,padding:"1rem"}}>
              <div style={{fontSize:".75rem",fontWeight:800,color:"var(--text-tertiary)"}}>الصف والشعبة</div>
              <div style={{marginTop:".4rem",fontSize:"1.05rem",fontWeight:900,color:"var(--primary)"}}>
                {[accountCard.class_name, accountCard.section ? `الشعبة ${accountCard.section}` : null].filter(Boolean).join(" • ") || "—"}
              </div>
            </div>
            <div style={{background:"#F8FBFF",border:"1px solid rgba(15,91,141,0.12)",borderRadius:18,padding:"1rem"}}>
              <div style={{fontSize:".75rem",fontWeight:800,color:"var(--text-tertiary)"}}>معرّف الدخول</div>
              <div style={{marginTop:".4rem",fontSize:"1rem",fontWeight:900,color:"var(--primary)",direction:"ltr",textAlign:"left"}}>{accountCard.login_identifier}</div>
            </div>
            <div style={{background:"#F8FBFF",border:"1px solid rgba(15,91,141,0.12)",borderRadius:18,padding:"1rem"}}>
              <div style={{fontSize:".75rem",fontWeight:800,color:"var(--text-tertiary)"}}>كلمة المرور المؤقتة</div>
              <div style={{marginTop:".4rem",fontSize:"1rem",fontWeight:900,color:"var(--primary)",direction:"ltr",textAlign:"left"}}>{accountCard.temporary_password}</div>
            </div>
          </div>
          <div style={{background:"#F8FBFF",border:"1px solid rgba(15,91,141,0.12)",borderRadius:18,padding:"1rem",marginBottom:"1rem"}}>
            <div style={{fontSize:".82rem",fontWeight:900,color:"var(--primary)",marginBottom:".5rem"}}>تعليمات الدخول</div>
            <ol style={{margin:0,paddingRight:"1.2rem",fontSize:".8rem",color:"var(--text-tertiary)",lineHeight:1.9}}>
