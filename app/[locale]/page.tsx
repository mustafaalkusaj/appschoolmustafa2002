  ...
  return (
    <ProtectedRoute roles={["super_admin", "admin", "employee"]}>
      <div className="min-h-screen bg-gradient-to-br from-sky-50 via-white to-blue-100 dark:from-slate-900 dark:via-slate-950 dark:to-slate-900">
        <header className="border-b border-[var(--border)]" style={{background:"var(--surface-strong)"}}>
          <div className="max-w-7xl mx-auto px-6 py-4 flex items-center justify-between">
            <UltrathinkLogo size={40} title="منصة إدارة المدرسة" subtitle="نظام التشغيل المدرسي" />
            <div className="text-end">
              <p className="text-sm text-[var(--text-secondary)]">مرحباً بعودتك</p>
              <p className="font-semibold text-[var(--text-primary)]">{profile?.email}</p>
            </div>
          </div>
        </header>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {cards.map((card) => (
              <Link key={card.href} href={localizeAppPath(card.href, locale)} className="group">
                <div className="content-card p-7 cursor-pointer transform hover:-translate-y-1 transition-all hover:shadow-lg">
                  <div className="mb-4">
                    <AppIcon token={card.icon} size={36} className="text-[var(--primary)]" />
                  </div>
                  <h2 className="text-xl font-bold text-[var(--text-primary)] mb-2 group-hover:text-[var(--primary)]">
                    {card.title}
                  </h2>
