...
export function usePaymentsPage(options?: { currentBranchId?: string | null }) {
  ...
  const printReceipt = useCallback(
    (p: Payment, student: Student) => {
      ...
      printHtmlDocument(
        wrapPrintDocument({
          title: isEnglish ? "Payment Receipt" : "إيصال دفعة",
          ...
          extraStyles: `@media print { @page { size: A5; margin: 1cm; } }`,
          bodyHtml: `
            <div class="print-panel" style="margin-bottom:16px">
              <div class="print-grid">
                ...
                <div>
                  ...
                  <div class="print-value">${escapeHtml(methodMap[p.payment_method as string] || p.payment_method || "—")}</div>
                </div>
              </div>
            </div>
            <div class="print-panel" style="margin-bottom:16px">
              <div class="print-grid">
                <div>
                  ...
                  <div class="print-value" style="font-size:15px">د.ع ${formatNumber(student.total_fee ?? 0)}</div>
                </div>
                ${(student.discount_value ?? 0) > 0 ? `<div>
                  <span class="print-label">${isEnglish ? "Discount" : "التخفيض"}</span>
                  <div class="print-value" style="font-size:15px;color:#d97706">د.ع ${formatNumber(student.discount_value ?? 0)}</div>
                </div>` : ""}
                <div>
                  <span class="print-label">${isEnglish ? "Remaining after payment" : "المتبقي بعد الدفعة"}</span>
                  <div class="print-value" style="font-size:15px;color:${remaining <= 0 ? "#16a34a" : "#dc2626"}">د.ع ${formatNumber(remaining)}</div>
                </div>
              </div>
            </div>
