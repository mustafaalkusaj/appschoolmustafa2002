...
import { Search, RefreshCw, Download, Plus, FileText, Tags, Trash2, Edit2, X, Printer, Loader2, TrendingUp, CalendarDays, Archive } from "@/lib/icons";
...
function AnimatedNumber({ value }: { value: number }) {
  ...
  useEffect(() => {
    const controls = motionAnimate(motionVal, value, { duration: 1, ease: "easeOut" });
...
export default function IncomesPage() {
  ...
  async function handleSaveIncome(e: React.FormEvent) {
    ...
    if (!response.ok) {
      ...
      setForm({ income_type_id: "", amount: "", income_date: DEFAULT_INCOME_DATE, source: "", receipt_number: "", notes: "", receipt_image_url: null });
  ...
  function openEditIncome(inc: IncomeRow) {
    ...
    setForm({
      ...
      notes: inc.notes || "",
      receipt_image_url: (inc as { receipt_image_url?: string | null }).receipt_image_url ?? null,
    });
    setShowIncomeForm(true);
  }

  const handleReceiptUploadIncome = async (e: React.ChangeEvent<HTMLInputElement>) => {
  ...
  async function exportPdf() {
    ...
    if (activeTab === "invoices") {
      ...
      const html = wrapPrintDocument({
        ...
        bodyHtml: `<table><thead><tr><th>#</th><th>${copy.exportRows.type}</th><th>${copy.exportRows.amount}</th><th>${copy.exportRows.date}</th><th>${copy.exportRows.source}</th><th>${copy.exportRows.receipt}</th></tr></thead><tbody>${rows.map((item, i) => `<tr><td>${i + 1}</td><td>${item.income_types?.name || "—"}</td><td>${currency} ${formatNumber(item.amount || 0)}</td><td>${item.income_date}</td><td>${item.source || "—"}</td><td>${item.receipt_number || "—"}</td></tr>`).join("")}</tbody></table>`,
  ...
  const statsCards = [
    { label: tx("إجمالي الإيرادات", locale), sublabel: tx("مجموع كل الإيرادات", locale), value: incomeSummary.schoolTotalAmount, displayValue: formatCurrencyWithSymbol(incomeSummary.schoolTotalAmount, currency), icon: TrendingUp, color: "var(--success)" },
    { label: tx("أنواع الإيرادات", locale), sublabel: tx("تصنيفات مختلفة", locale), value: incomeTypes.length, displayValue: null, icon: Tags, color: "var(--info)" },
    { label: tx("إيرادات اليوم", locale), sublabel: tx("إجمالي اليوم الحالي", locale), value: incomeSummary.schoolTodayAmount, displayValue: formatCurrencyWithSymbol(incomeSummary.schoolTodayAmount, currency), icon: CalendarDays, color: "var(--warning)" },
  ];

  const tabs: { id: "invoices" | "types"; label: string; icon: React.ElementType; color: string }[] = [
    { id: "invoices", label: `${copy.invoices} (${incomeSummary.schoolTotalCount})`, icon: FileText, color: "var(--success)" },
    { id: "types",    label: `${copy.categories} (${incomeTypes.length})`,           icon: Tags,     color: "var(--primary)" },
  ];
              ...
              {archiveMode.isArchiveMode && archiveMode.archiveData && (
                <div className="rounded-2xl flex items-center gap-3 px-5 py-3.5 border border-indigo-500/30"
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-sm font-black text-white">عرض إيرادات سنة {archiveMode.archiveData.year} · يمكنك تصدير البيانات لكن لا يمكن تعديلها</p>
                  </div>
                    ...
                    <div className="relative flex items-center justify-between gap-4">
                      <div>
                        <p className="text-white/60 text-xs font-medium mb-2 tracking-widest uppercase">
                          {isEnglish ? "Administration · Incomes" : "لوحة الإدارة · الإيرادات"}
                        </p>
                        <h1 className="text-2xl md:text-3xl font-black text-white mb-1.5 leading-tight">
                          {isEnglish ? "Incomes" : "الإيرادات"}
                        </h1>
                        <p className="text-white/70 text-sm">
                          {isEnglish ? "Manage income records and categories" : "إدارة سجلات الإيرادات الدراسية وأنواعها"}
                        </p>
                      </div>
                      <div className="hidden md:flex items-center justify-center w-20 h-20 rounded-2xl flex-shrink-0" style={{ background: "rgba(255,255,255,0.15)", backdropFilter: "blur(8px)" }}>
                        <TrendingUp size={38} className="text-white" />
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="relative rounded-2xl border border-[var(--border)] bg-[var(--card-bg)] p-1.5 flex gap-1">
                    {tabs.map((tab) => {
                      ...
                      return (
                        ...
                        >
                          {isActive && (
                            <motion.div layoutId="incomes-tab-pill" className="absolute inset-0 rounded-xl"
                    ...
                    <div className="flex flex-wrap items-center justify-between gap-3">
                      {activeTab === "invoices" && (
                        <div className="flex flex-wrap items-end gap-3 flex-1">
                          ...
                          <div className="flex flex-col gap-1">
                            <label className="text-[10px] font-black uppercase tracking-wider text-[var(--text-muted)]">{tx("نوع الإيراد", locale)}</label>
                            <select className="h-9 rounded-xl border border-[var(--border)] bg-[var(--surface-strong)] px-3 text-sm text-[var(--text-primary)] outline-none focus:border-[var(--primary)] transition" value={incomeTypeFilter} onChange={e => setIncomeTypeFilter(e.target.value)}>
                      ...
                      <div className="flex items-center gap-2 flex-wrap flex-shrink-0">
                        ...
                        {!archiveMode.isArchiveMode && (
                          <motion.button
                            ...
                            onClick={() => {
                              if (activeTab === "invoices") { setEditIncome(null); setForm({ income_type_id: "", amount: "", income_date: new Date().toISOString().split("T")[0], source: "", receipt_number: "", notes: "", receipt_image_url: null }); setShowIncomeForm(true); }
                      ...
                      >
                        ...
                        <div className="flex items-center justify-between gap-3">
                          ...
                          <span className="text-xs font-bold text-[var(--text-muted)]">
                            {formatRecordCount(activeTab === "invoices" ? incomeTotalCount : filteredTypes.length, locale)}
                          </span>
                        </div>
                          ...
                          incomesLoading ? (
                            ...
                            <>
                                  ...
                                  <tbody className="divide-y divide-[var(--border)]">
                                    {incomes.map((e, i) => (
                                      <tr key={e.id} className="hover:bg-[var(--surface-soft)] transition-colors">
                                        ...
                                        <td className="p-4 text-sm font-black text-[var(--success)]">{formatCurrencyWithSymbol(e.amount, currency)}</td>
                                        <td className="p-4 text-xs font-bold text-[var(--text-secondary)]">{formatDate(e.income_date)}</td>
                                        <td className="p-4 text-xs font-bold text-[var(--text-secondary)]">{e.source || "—"}</td>
                                        <td className="p-4 text-xs font-black text-[var(--primary)] uppercase">{e.receipt_number || "—"}</td>
                                        <td className="p-4">
                                          {!archiveMode.isArchiveMode && (
                                            <div className="flex gap-2">
                                              <button className="h-8 w-8 flex items-center justify-center rounded-lg bg-[var(--primary)]/10 text-[var(--primary)] hover:bg-[var(--primary)]/20 transition-colors" onClick={() => openEditIncome(e)}>
                                                <Edit2 size={14} />
                                              </button>
                                              <button className="h-8 w-8 flex items-center justify-center rounded-lg bg-[var(--danger)]/10 text-[var(--danger)] hover:bg-[var(--danger)]/20 transition-colors" onClick={() => setPendingDelete({ type: "income", id: e.id })}>
                                                <Trash2 size={14} />
                                              </button>
                                            </div>
                                          )}
                              ...
                              <div className="flex flex-col sm:flex-row items-center justify-between gap-3 pt-2">
                                <div className="flex items-center gap-2 px-4 py-2 rounded-xl bg-[var(--success)]/10 text-[var(--success)]">
                                ...
                                <div className="flex items-center gap-2">
                                  <button className="h-9 px-4 rounded-xl border border-[var(--border)] bg-[var(--surface-soft)] text-xs font-black disabled:opacity-40 hover:bg-[var(--card-bg)] transition-colors" onClick={() => setIncomePage(c => Math.max(1, c - 1))} disabled={incomePage <= 1}>{tx("السابق", locale)}</button>
                                  <span className="text-xs font-bold text-[var(--text-secondary)]">{formatPageLabel(incomePage, incomeTotalPages, locale)}</span>
                                  <button className="h-9 px-4 rounded-xl border border-[var(--border)] bg-[var(--surface-soft)] text-xs font-black disabled:opacity-40 hover:bg-[var(--card-bg)] transition-colors" onClick={() => setIncomePage(c => Math.min(incomeTotalPages, c + 1))} disabled={incomePage >= incomeTotalPages}>{tx("التالي", locale)}</button>
                                </div>
                              </div>
                            </>
                          )
            ...
            <form onSubmit={handleSaveIncome} className="p-8 space-y-5">
              ...
              <div className="space-y-1.5">
                ...
                <label className="flex items-center gap-2 cursor-pointer h-10 px-3 rounded-xl border border-dashed border-[var(--border)] bg-[var(--surface-soft)] text-sm text-[var(--text-muted)] hover:border-[var(--primary)] hover:text-[var(--primary)] transition-colors w-full">
                  ...
                  <input type="file" accept="image/jpeg,image/png,image/webp" className="hidden" onChange={handleReceiptUploadIncome} disabled={uploadingReceipt} />
                </label>
