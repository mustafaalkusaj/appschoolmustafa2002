  ...
  try {
    const payload = schoolId
      ? await rememberWithTtl(
          `teacher-activity:homework:${schoolId}:${request.nextUrl.searchParams.toString()}`,
          15_000,
          async () => {
    ...
    const status = error instanceof Error && "status" in error && typeof error.status === "number"
      ? error.status
      : 500;
    return jsonError(error instanceof Error ? error.message : "تعذر تحميل الواجبات.", status);
  }
}
