...
function generateUsername(employeeId?: string | null): string {
  const digits = Math.floor(1000 + Math.random() * 9000).toString();
  if (employeeId) {
    const clean = employeeId.toLowerCase().replace(/[^a-z0-9-]/g, "").slice(0, 12);
    if (clean) return `t.${clean}.${digits}`;
  }
  return `teacher.${digits}`;
}
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    for (const teacher of teachers) {
      ...
      const username = generateUsername(teacher.employee_id as string | null);
