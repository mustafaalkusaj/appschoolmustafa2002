...
function buildRows(branch: SchoolManagerBranchSummary): Array<[string, string]> {
  return [
    ...
    ["الصافي الشامل", formatCurrency(branch.totalPaid + branch.totalIncomes - branch.totalExpenses)],
    ...
    const sheets = [
      ...
      ...(!selectedBranch ? [{
        ...
        rows: overview.branches.map((branch) => ({
          ...
          netIncome: branch.totalPaid + branch.totalIncomes - branch.totalExpenses,
