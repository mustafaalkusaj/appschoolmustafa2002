...
-- ============================================================
-- 1. DEDUCTIONS (school-level only — no branch_id column)
-- ============================================================
      ...
      SELECT 1 FROM user_profiles up
       WHERE up.id = auth.uid()
         AND (
           ...
           OR (up.scope_level IN ('group_admin', 'branch_user', 'restricted') AND deductions.school_id = up.school_id)
         )
    )
  );
    ...
    EXISTS (
      SELECT 1 FROM user_profiles up
       WHERE up.id = auth.uid()
         AND (
           up.scope_level = 'super_admin'
           OR (up.scope_level IN ('group_admin', 'branch_user') AND deductions.school_id = up.school_id)
         )
    )
  );
