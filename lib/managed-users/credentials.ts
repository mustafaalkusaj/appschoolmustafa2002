...
export async function generateManagedLoginIdentifier(
  ...
  const { isMissingTableError: checkMissingTable } = await import("@/lib/admin-infrastructure");
  ...
  for (let attempt = 0; attempt < 10; attempt += 1) {
    ...
    if (managedProfilesProbe.error && !checkMissingTable(managedProfilesProbe.error, "managed_user_profiles")) {
      throw managedProfilesProbe.error;
    }

    const profileMatch =
      managedProfilesProbe.error && checkMissingTable(managedProfilesProbe.error, "managed_user_profiles")
        ? await actorSupabase
            .from("user_profiles")
            .select("id")
            .eq("school_id", options.schoolId)
